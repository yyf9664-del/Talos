...
Return the chart inline in the chat using Markdown image syntax:

```md
![Generated chart](<image_url>)
```

Then include the complete `args` (specification) used for generation.
Do not create a local image file or return the chart as an artifact/file-panel preview unless the user explicitly asks for a downloadable file.
