...
class FeishuChannel(BaseChannel):
    ...
    def _remove_reaction_sync(self, message_id: str, reaction_id: str) -> None:
        ...
        try:
            ...
            if response.success():
                logger.debug("Removed reaction %s from message %s", reaction_id, message_id)
            else:
                logger.debug(
                    "Failed to remove reaction: code={}, msg={}", response.code, response.msg
                )
        except Exception as e:
            logger.debug("Error removing reaction: %s", e)
    ...
    def _create_streaming_card_sync(self, receive_id_type: str, chat_id: str) -> str | None:
        ...
        try:
            ...
            logger.warning("Error creating streaming card: %s", e)
            return None
    ...
    def _stream_update_text_sync(self, card_id: str, content: str, sequence: int) -> bool:
        ...
        try:
            ...
            logger.warning("Error stream-updating card %s: %s", card_id, e)
            return False
    ...
    def _close_streaming_mode_sync(self, card_id: str, sequence: int) -> bool:
        ...
        try:
            ...
            logger.warning("Error closing streaming on card %s: %s", card_id, e)
            return False
