...
<p align="center">
  <img src="docs/readme/openyak-workflow-artifacts.gif" width="900" alt="Talos 把多份办公文件整理成结构化回答和可复用 artifact" />
</p>
...
Talos 不是另一个必须登录的云端工作区，而是一个运行在你电脑上的本地 AI agent。

- **不需要 Talos 账号。** 安装应用，选择本地模型或自带 provider key，就可以开始工作；没有登录、账单、席位或充值流程。
...
Talos 可以把很长的 memo 整理成给管理层、团队同步或 follow-up 邮件使用的结构化 brief。

<p align="center">
  <img src="docs/readme/openyak-memo-to-brief.gif" width="900" alt="Talos 从 memo 生成高管简报的动线" />
</p>

<p align="center">
  <img src="docs/readme/openyak-docx-brief.png" width="900" alt="Talos DOCX memo review 结果细节" />
</p>

### 从表格到财务口径

表格不应该只被截图摘要。你可以要求 Talos 分析预算差异、forecast 风险、owner 级行动项，以及可以直接拿去开会的财务口径。

<p align="center">
  <img src="docs/readme/openyak-budget-analysis.png" width="900" alt="Talos 表格预算分析结果细节" />
</p>

### 从多文件到 Artifact

Talos 可以在同一个线程里综合多份文件，并在右侧 artifact panel 打开可复用的 brief、计划、图表和结构化输出。

<p align="center">
  <img src="docs/readme/openyak-artifact-panel.png" width="900" alt="Talos 右侧 artifact panel 展示多文件 board brief" />
</p>
...
<p align="center">
  <img src="docs/readme/openyak-multi-agent-task-batch.png" width="900" alt="Talos 并行 multi-agent task batch，展示子会话链接和汇总结果" />
</p>

### 长对话与自动压缩

真实办公任务很少一轮结束。Talos 支持连续追问、修订、长线程保留上下文，让任务从分析自然推进到执行。

<p align="center">
  <img src="docs/readme/openyak-auto-compress.gif" width="900" alt="Talos 长上下文自动压缩动线" />
</p>

<p align="center">
  <img src="docs/readme/openyak-long-context.png" width="900" alt="Talos 长对话保留上下文界面" />
</p>
...
- **Rapid-MLX：** Apple Silicon macOS 用户可以在设置里启动、切换精选 MLX 模型。Talos 会连接 Rapid-MLX 在 `localhost` 暴露的 OpenAI-compatible API。
- **Ollama：** 通过 [Ollama](https://ollama.com) 运行任意本地模型。Talos 会自动检测本地模型，也可以在无网络环境下工作。
- **自定义本地 endpoint：** 如果你自己运行 OpenAI-compatible 模型服务，可以直接把 Talos 指向本地地址。
...
云端和订阅路径都是可选项。Talos 不提供内置模型账号，也不代理模型流量；请求会从你的桌面端直接发往你配置的 provider。
...
- **隐私控制：** 本地存储、无需 Talos 账号、BYOK provider、本地模型支持。
...
<details>
<summary>需要 Talos 账号吗？</summary>

不需要。Talos 不需要账号、登录、账单资料、充值流程、团队工作区或托管 Talos 后端。使用云端 provider 时，你需要自己的 API Key 或已有订阅；不使用云端 provider 时，可以直接走本地模型。
</details>

<details>
<summary>和 ChatGPT 或 Claude.ai 有什么区别？</summary>

Talos 运行在你的桌面上，围绕本地文件、artifact、工具、权限和连续工作流设计。网页版聊天助手很适合问答，Talos 更像一个能查看文件、使用工具、把长任务留在你电脑上的本地 agent 工作台。
</details>

<details>
<summary>可以离线使用吗？</summary>

可以。在 Apple Silicon macOS 上，可以使用 Rapid-MLX 和已下载的 MLX 模型；在 macOS、Windows 或 Linux 上，可以安装 Ollama 并下载模型。之后 Talos 可以在不调用云端模型的情况下本地运行。
</details>

<details>
<summary>远程访问怎么工作？</summary>

在设置里开启远程访问，扫描二维码即可打开移动端网页。Talos 通过 Cloudflare Tunnel 和 token-based authentication 连接，不需要端口转发。
</details>
