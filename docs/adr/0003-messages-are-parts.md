...
A Message in Talos is not a string with a role tag — it is an ordered list of **Parts** (text, reasoning, tool, step-finish, compaction, subtask, file). The Part type is a discriminated union persisted as JSON (`backend/app/schemas/message.py`), and the frontend renders each Part type with a dedicated component (`frontend/src/components/parts/`).
